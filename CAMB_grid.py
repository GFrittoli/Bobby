'''
File containing functions to grid Euclid-like distributions (mimicking Ilich +2022). This file is an example valid for General relativity.
...
- find_s8: Find the value of sigma8 for a given set of parameters.
...
def dndz_bin(z, z_med, z_bin_m, z_bin_p, c_b, z_b, sigma_b, c_0, z_0, sigma_0, f_out):
    """
    Calculate the redshift distribution for a given bin.

    Parameters:
    - z: Redshift value or array of redshift values.
    - z_med: Median redshift.
    - z_bin_m: Minimum redshift bin edge.
    - z_bin_p: Maximum redshift bin edge.
    - c_b, z_b, sigma_b, c_0, z_0, sigma_0, f_out: Parameters for the redshift distribution function.

    Returns:
    - Redshift distribution for the given bin.
    """
    z0 = z_med / np.sqrt(2)  # Calculate z0
    dndz_tot = dndz(z, z0)  # Calculate total dndz
    
    # Calculate the dndz for the bin using the given parameters
